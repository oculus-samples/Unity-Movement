...
namespace Meta.XR.Movement.Editor
{
    ...
    public static class BuildSamples
    {
        ...
        // Sample folders
        private const string _bodyTrackingFolder = "BodyTrackingSamples";
        private const string _faceTrackingFolder = "FaceTrackingSamples";
        private const string _advancedSamplesFolder = "AdvancedSamples";
        ...
        private static string FindScenePath(string sampleFolder, string scenePath)
        {
            ...
            var fullPath = Path.Combine(packagePath, "Samples", sampleFolder, scenePath);
            if (Directory.Exists(packagePath) && File.Exists(fullPath))
            {
                return $"Packages/com.meta.xr.sdk.movement/Samples/{sampleFolder}/{scenePath}";
            }
